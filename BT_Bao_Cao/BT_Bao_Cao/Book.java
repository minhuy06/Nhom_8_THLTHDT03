...
public abstract class Book implements IGiaBan, IKiemKe, IPrintTable {
	...
	public void printTableRow() {
		String title = getTitle().length() > 35 ? getTitle().substring(0, 32) + "..." : getTitle();
		String author = getAuthor().length() > 18 ? getAuthor().substring(0, 15) + "..." : getAuthor();
		System.out.printf("%-10s %-35s %-18s %-6d %-8d %-10.2f%n", getBook_ID(), title, author, getPub_year(),
				getQuantity(), tinhGiaBan());
	}
	...
	public String toString() {
		return "Book [book_ID=" + book_ID + ", title=" + title + ", author=" + author + ", pub_year=" + pub_year
				...
				+ ", getStatus()=" + getStatus() + ", tinhGiaBan()=" + tinhGiaBan() + "]";
	}
